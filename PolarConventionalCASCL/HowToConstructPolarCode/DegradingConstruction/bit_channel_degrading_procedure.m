...
if N == 1
    ...
    W_up_after_merge = degrading_merge(W_up_after_erasure_symbol_merge, miu);
    ...
    W_down_after_merge = degrading_merge(W_down_after_erasure_symbol_merge, miu);
...
%             W_up = LR_sort(W_up);
%             W_up_after_erasure_symbol_merge = erasure_symbol_merge(W_up);
%             W_tmp = degrading_merge(W_up_after_erasure_symbol_merge, miu);
%         else
%             W_down = get_W_down(W_tmp);
%             W_down = LR_sort(W_down);
%             W_down_after_erasure_symbol_merge = erasure_symbol_merge(W_down);
%             W_tmp = degrading_merge(W_down_after_erasure_symbol_merge, miu);
%             
%         end
%     end
%     Pe(k + 1) = 0.5 * sum(min(W_tmp));
%     disp(['Bit index = ' num2str(k + 1) '  Error rate = ' num2str(Pe(k + 1))])
% end
% end
